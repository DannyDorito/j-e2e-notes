  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
      {notes.filter( ( note ) => !note.deleted ).map( ( note ) => (
        <DraggableNote note={note} key={note.id} deleteNote={() => deleteNote( note.id )} editNote={() => editNote( note.id )}></DraggableNote>
      ) )
      }
      <IconButton size={'large'} style={{ position: "fixed", bottom: 0, right: 0 }} onClick={addNote}><AddCircleOutlineIcon /></IconButton>
    </Box>
  );
}
